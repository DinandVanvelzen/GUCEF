...
#ifndef GUCEF_CORE_CCONSTRAINTREGISTRY_H
#define GUCEF_CORE_CCONSTRAINTREGISTRY_H
...
/**
 *  Registry for constraint implementations
 */
class GUCEF_CORE_PUBLIC_CPP CFunctionRegistry
{
    public:

    CFunctionRegistry( void );

    CFunctionRegistry( const CFunctionRegistry& src );

    virtual ~CFunctionRegistry();

    CFunctionRegistry& operator=( const CFunctionRegistry& src );

    CIFunction* GetFunctionByName( const CString& functionName ) const;

    void RegisterBasicBuildInFunctions( void );
...
#endif /* GUCEF_CORE_CCONSTRAINTREGISTRY_H ? */
